...
public class RouteController {
    ...
    public ResponseEntity<?> create(@Valid @RequestBody RouteDTO routeDTO) {
        ...
        if (airportOriginDTO.getIata() == null) {
            return new ResponseEntity<>("El codigo iata del aeropuerto de origen no debe ser nulo", HttpStatus.BAD_REQUEST);
        }
        ...
        if (airportDestinationDTO.getIata() == null) {
            return new ResponseEntity<>("El codigo iata del aeropuerto de destino no debe ser nulo", HttpStatus.BAD_REQUEST);
        }

        if (airportOriginDTO.getIata().length() < 2 || airportOriginDTO.getIata().length() > 3) {
            return new ResponseEntity<>("El codigo iata de origen debe contener entre 2 y 3 caracteres", HttpStatus.BAD_REQUEST);
        }

        if (airportDestinationDTO.getIata().length() < 2 || airportDestinationDTO.getIata().length() > 3) {
            return new ResponseEntity<>("El codigo iata de destino debe contener entre 2 y 3 caracteres", HttpStatus.BAD_REQUEST);
        }
    ...
    public ResponseEntity<?> getRouteByOriginAndDestinationAirport(@PathVariable("iataOrigin") String iataOrigin, @PathVariable("iataDestination") String iataDestination) {
        Airport airportOrigin = this.airportService.findByIata(iataOrigin);
        Airport airportDestination = this.airportService.findByIata(iataDestination);
        if (airportOrigin == null) {
    ...
    public ResponseEntity<?> delete(@PathVariable("iataOrigin") String iataOrigin, @PathVariable("iataDestination") String iataDestination) {
        Airport airportOrigin = this.airportService.findByIata(iataOrigin);
        Airport airportDestination = this.airportService.findByIata(iataDestination);

        if (airportOrigin == null) {
            return new ResponseEntity<>("No existe un aeropuerto de origen con el codigo iata especificado", HttpStatus.BAD_REQUEST);
        }

        if (airportDestination == null) {
            return new ResponseEntity<>("No existe un aeropuerto de destino con el codigo iata especificado", HttpStatus.BAD_REQUEST);
        }

        Route route = this.routeService.findByOriginAndDestination(airportOrigin, airportDestination);

        if (route == null) {
            return new ResponseEntity<>("No existe la ruta con esa iata de origen y esa de destino", HttpStatus.BAD_REQUEST);
        }
    ...
    public ResponseEntity<?> update(@PathVariable("iataOrigin") String iataOrigin, @PathVariable("iataDestination") String iataDestination,
                                    @Valid @RequestBody RouteDTO routeDTO) {
        Airport airportOrigin = this.airportService.findByIata(iataOrigin);
        if (airportOrigin == null) {
            return new ResponseEntity<>("No existe un aeropuerto con el codigo iata especificado", HttpStatus.BAD_REQUEST);
        }
        Airport airportDestination = this.airportService.findByIata(iataDestination);
        if (airportDestination == null) {
            return new ResponseEntity<>("No existe un aeropuerto con el codigo iata especificado", HttpStatus.BAD_REQUEST);
        ...
        if (route == null) {
            return new ResponseEntity<>("No existe un ruta con esa iata de origen y esa iata de destino", HttpStatus.BAD_REQUEST);
        } else {
            ...
            if (iataOriginDTO != null && iataDestinationDTO != null) {
                if (iataOriginDTO.length() < 2 || iataOriginDTO.length() > 3 && iataDestinationDTO.length() < 2 || iataDestinationDTO.length() > 3) {
